...
function Leadership() {
  const roles = [
    ...
    { title: 'Editorial Team Member', org: 'Various Publications', desc: 'Contributing to technical editorial processes and content curation.' },
    { title: 'Peer Reviewer', org: 'Scopus-Indexed Journals', desc: 'Reviewing AI/ML research for academic journals.' },
  ];

  return (
    <section id="leadership" className="bg-gradient-to-b from-darker via-[#051020] to-darker px-5 py-24">
      <div className="relative z-10 mx-auto max-w-6xl">
        ...
        <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
          {roles.map((item) => (
            <div key={item.title} className="rounded-xl border border-neon-cyan/20 bg-[#071426]/80 p-5 transition-all hover:-translate-y-1 hover:border-neon-cyan/45">
              <h3 className="mt-1 text-lg font-semibold text-neon-cyan">{item.title}</h3>
